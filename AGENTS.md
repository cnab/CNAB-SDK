...
The published docs live at **https://cnab.github.io/** (landing page) and
**https://cnab.github.io/api/** (TypeDoc reference), served from the
[`cnab/cnab.github.io`](https://github.com/cnab/cnab.github.io) repo.

**This repo does not publish a Pages site**, deliberately. It used to try, via a
`pages.yml` that failed on all 8 of its runs. The reason is structural: enabling
Pages on a project repo needs a repository admin, and `GITHUB_TOKEN` cannot do
it — `pages: write` authorises *deploying to* an existing site, not *creating*
one, so `actions/configure-pages` with `enablement: true` gets
`Resource not accessible by integration` no matter what the workflow requests.
An `<org>.github.io` repo has none of that problem: it serves from its default
branch automatically. One site, one place, no admin step.

What this repo still does:

- `npm run docs` builds the TypeDoc site into `site/` (gitignored). It needs
  `npm run build:spec` first — `src/spec.generated.ts` is generated, and without
  it TypeDoc cannot resolve the import and emits nothing.
- The **`docs` job in CI** runs both on every push and PR and asserts the site
  contains `index.html` plus a page for each of the six engine classes. That is
  what stops the docs build rotting again between releases.

Publishing is a step in the release runbook below, not an automatic push,
because it crosses repositories and needs a token this workflow does not have
(#19).
...
are generated by Changesets. Add a `## [X.Y.Z]` section **before** the version PR
merges — `changelog-section.mjs` fails the release rather than publish a release
with empty notes.

### After the release: refresh the published docs

The site is a separate repo and nothing pushes to it automatically, so this is a
manual step until #19 provides a cross-repo token:

```bash
git clone https://github.com/cnab/cnab.github.io
cd cnab.github.io
./tools/sync-api-docs.sh ../CNAB-SDK
git add api && git commit -m "docs: sync API reference to vX.Y.Z" && git push
```

Also update the version and counts on that repo's `index.html` — they are claims
about the software, and a stale claim is worse than no claim.
